...
static void MakeTexture(const std::string& path, Color base, Color accent, int mode) {
    Image img = GenImageColor(64, 64, base);
    ImageDrawRectangle(&img, 0, 0, 63, 3, accent);
    ImageDrawRectangle(&img, 0, 60, 63, 3, accent);
    for (int y = 0; y < 64; y += 8) {
        for (int x = 0; x < 64; x += 8) {
            if ((x / 8 + y / 8 + mode) % 3 == 0)
                ImageDrawRectangle(&img, x + 1, y + 1, 5, 5, Color{accent.r, accent.g, accent.b, 100});
        }
    }
...
bool GenerateAssets(const std::string& root, const std::function<void(int, const std::string&)>& progress) {
    try {
        const fs::path dir = fs::path(root) / "assets" / "generated";
        fs::create_directories(dir);
        const struct T { const char* name; Color a; Color b; int m; } textures[] = {
        ...
        const int count = static_cast<int>(sizeof(textures) / sizeof(textures[0]));
        for (int i = 0; i < count; ++i) {
            progress(10 + i * 9, std::string("Generating ") + textures[i].name);
            MakeTexture((dir / textures[i].name).string(), textures[i].a, textures[i].b, textures[i].m);
        }
